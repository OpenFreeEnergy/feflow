...
def generate_endpoint_thermodynamic_states(system, topology_proposal, repartitioned_endstate=None,
    ...
    nonalchemical_zero_thermodynamic_state = states.ThermodynamicState(topology_proposal.old_system,
                                                                       temperature=temperature)
    nonalchemical_one_thermodynamic_state = states.ThermodynamicState(topology_proposal.new_system,
                                                                      temperature=temperature)
    ...
    if repartitioned_endstate == 0:
        ...
        lambda_zero_thermodynamic_state = (
            states.CompoundThermodynamicState(thermodynamic_state,
                                              composable_states=[lambda_zero_alchemical_state]))
        lambda_one_thermodynamic_state = (
            states.CompoundThermodynamicState(thermodynamic_state,
                                              composable_states=[lambda_one_alchemical_state]))

    return (nonalchemical_zero_thermodynamic_state, nonalchemical_one_thermodynamic_state,
