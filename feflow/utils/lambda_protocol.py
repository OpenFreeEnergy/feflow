    ...
    * Class needs cleaning up and made more consistent
    ...
    def _validate_schedule(schedule, windows):
        ...
        if schedule[0] != 0 or schedule[-1] != 1:
            errmsg = (
                "end and start lambda windows must be lambda 0 and 1 " "respectively"
            )
            raise ValueError(errmsg)
    ...
    def _validate_functions(self, n=10):
        ...
        for function in required_functions:
            if function not in self.functions:
                ...
                errmsg = (
                    f"function {function} is missing from " "self.lambda_functions."
                )
                raise ValueError(errmsg)
            ...
            if not all(i >= 0.0 for i in difference):
                wmsg = (
                    f"The function {function} is not monotonic as "
                    "typically expected."
                )
                warnings.warn(wmsg)
...
class RelativeAlchemicalState(AlchemicalState):
    ...
    def set_alchemical_parameters(
        self, global_lambda, lambda_protocol=LambdaProtocol()
    ):
