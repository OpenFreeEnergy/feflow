...
from fenchiridion.protocols import NonEquilibriumCyclingProtocol
...
class TestNonEquilibriumCycling:
    ...
    def test_create_execute_gather_toluene_to_toluene(self, protocol, toluene_vacuum_system, mapping_toluene_toluene, tmpdir, request):
        ...
        assert np.isclose(fe_estimate.magnitude, 0.0, atol=1e-10), f"Free energy estimate {fe_estimate} is not close to zero."
